            ...
            .pipe(
                catchError((errorResponse: HttpErrorResponse) => {
                    ...
                    if (errorResponse.status === 401 && error.error === 'token_expired' || (errorResponse.status === 0)) {
                        ...
                        //     .pipe(
                        //         flatMap(data => {
                        //             localStorage.setItem("dG9rZW5fc2FnZXNj", data.token)
                        //             const authRequest = request.clone({ setHeaders: { 'Authorization': `Bearer ${data.dG9rZW5fc2FnZXNj}` } })
                        //             return next.handle(authRequest)
                        //         })
                        //     )
                    }
                    // console.log("token status"+errorResponse.status);
                    if (errorResponse.status === 400 && (error.error === 'token_expired' || error.error === 'token_invalid' || error.error === 'A token is required' || error.error === 'token_not_provider')) {
                        const loginService = this.injector.get(LoginService);
