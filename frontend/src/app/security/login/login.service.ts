...
// import { PermissaoGrupoService } from '../../permissao-grupo/permissao-grupo.service'
...
export class LoginService {
  ...
  getUser() {
    ...
    return JSON.parse(atob(this.helper.decrypt(user)));
  }
  checkPermission(route) {
    // let user = this.getUser();
    // return this.permissaoGrupoService.checkPermission(route);
  }

  isLoggedIn(): boolean {
    if (localStorage.getItem("dG9rZW5fc2FnZXNj") !== null) {
  ...
  logoutForce() {
    localStorage.removeItem("dG9rZW5fc2FnZXNj");
  ...
  login(login: string, password: string): Observable<User> {
    return this.http
      .post<User>(`${API}/auth/login`, { login: login, password: password })
      .pipe(
        tap((user) => {
          localStorage.setItem("dG9rZW5fc2FnZXNj", user.token);
