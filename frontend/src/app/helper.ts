...
export class Helper {
	...
	extensoesProibidas(event) {
		let extensao = this.getExtensionOfFile(event.target.files[0].name);
		var deny = ["html", "exe", "js", "py", "php", "cc", "cpp", "cxx", "c", "c++", "h", "hh", "hpp", "hxx", "h++", "c++", "jar", "java", "deb", "tar", "zip"];
		return deny.indexOf(extensao.toLowerCase()) > -1;
	}
	extensoesValidasAnexos(event) {
		let extensao = this.getExtensionOfFile(event.target.files[0].name);
		var days = ["pdf", "png", "jpg", "jpeg", , "doc", "docs", "docx", "xlsx", "xls"];
	...
	onFileChanged(event) {
		if (this.extensoesProibidas(event)) {
			this.notificationService.notify("Arquivo Proibido");
			return false;
		}
	...
	orderby(column,ar,order) {
		...
		Object.keys(order).forEach(function(key) {
			if(column == key){
				ar.sort((a, b) => (a[key] > b[key]) ? order[key] == 'desc' ? -1 : 1 : order[key] == 'desc' ? 1 : -1)
